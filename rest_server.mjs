...
app.get('/incidents', (req, res) => {
    ...
    if ("code" in req.query) {
        let safe_codes = req.query["code"].split(",").map(c => parseInt(c)).join(", ");
        whereParts.push(`code in (${safe_codes})`);
    }
    if ("grid" in req.query) {
        let safe_grid = req.query["grid"].split(",").map(g => parseInt(g)).join(", ");
        whereParts.push(`police_grid in (${safe_grid})`);
    }
    if ("neighborhood" in req.query) {
        let safe_neighborhood = req.query["neighborhood"].split(",").map(n => parseInt(n)).join(", ");
